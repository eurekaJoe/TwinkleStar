Shader "Skybox/SkyShader"   // https://halisavakis.com/my-take-on-shaders-sky-shader/
{
    Properties
    {
        [Header(Sky color)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 uv = float2(atan2(i.uv.x, i.uv.z) / UNITY_TWO_PI, asin(i.uv.y) / UNITY_HALF_PI);//calculating the UVs for the skybox in a manner that eliminates annoying stretching
                ...
                float cloudsThreshold = i.uv.y - _CloudsThreshold;  //adjust how dissolved the clouds get the further up they are
