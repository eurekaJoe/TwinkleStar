...
public class MeshPainterStyle : Editor
{
    ...
    bool Cheak()
    {
        ...
        if(Select.gameObject.GetComponent<MeshRenderer>().sharedMaterial.shader == Shader.Find("Mya/texBlend/mya_4tex_blend_diffuce") || Select.gameObject.GetComponent<MeshRenderer>().sharedMaterial.shader == Shader.Find("Custom/TexBlend"))
        {
            if(ControlTex == null)
            {
    ...
    void Painter()
    {
        ...
        if (Physics.Raycast(terrain, out raycastHit, Mathf.Infinity, 1 << LayerMask.NameToLayer("Mask")))//射线检测名为"Mask"的层
        {
            ...
            if ((e.type == EventType.MouseDrag && e.alt == false && e.control == false && e.shift == false && e.button == 0) || (e.type == EventType.MouseDown && e.shift == false && e.alt == false && e.control == false && e.button == 0 && ToggleF == false))
            {
                ...
                Texture2D TBrush = brushTex[selBrush] as Texture2D;//获取笔刷形状贴图
