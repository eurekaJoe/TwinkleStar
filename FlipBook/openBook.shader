...
// https://zhuanlan.zhihu.com/p/28836892   利用GPU实现翻页效果
...
Shader "Custom/FlipBookShader"
{
	...
	SubShader
	{
		...
		float4 flip_book(float4 vertex)
		{
			...
			float flipCurve = exp(-0.1 * pow(vertex.x - 0.5, 2)) * _CurPageAngle;	// 高斯函数
		...
		{
			Cull Front
			Offset -1, -1	// 在翻书和前一页快重合时，因为第二个pass中的某些顶点的深度要大于第一个pass的深度，从而造成穿帮
